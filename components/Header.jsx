...
const Header = () => {
    ...
    return (
        <header className="sticky top-0 z-[100] w-full bg-white/95 backdrop-blur-xl border-b border-rose/10 shadow-sm transition-all duration-300">
            <nav className="flex items-center w-full max-w-[90rem] mx-auto justify-between px-6 py-4 md:py-6">
                ...
                <div className="flex-shrink-0 flex justify-center flex-[2] md:flex-1">
                    <Link href="/" className="group flex flex-col items-center">
                        <span className="text-3xl md:text-5xl font-black text-[#fbcfe8] [text-shadow:_0_1px_3px_rgb(0_0_0_/_30%)] tracking-tighter font-serif drop-shadow-sm group-hover:scale-105 transition-transform">
                            Noury Beauty
                        </span>
                    </Link>
                </div>
                ...
                {/* Mobile Menu Sidebar - Glassmorphism */}
                <AnimatePresence>
                    {mobileOpen && (
                        <>
                            <motion.div 
                            ...
                            <motion.div 
                                ...
                                className="fixed inset-y-0 right-0 w-[280px] bg-pink-50 z-[110] shadow-2xl border-l border-rose/10 py-12 px-8 flex flex-col"
                            >
                                <button onClick={() => setMobileOpen(false)} className="absolute top-6 right-6 p-2 text-black hover:bg-rose/10 rounded-full transition-colors">
                                ...
                                <div className="mt-12 space-y-6 text-right">
                                    ...
                                    <div className="flex flex-col gap-4 text-black font-black uppercase tracking-widest text-lg">
                                        <Link className="hover:text-rose transition-colors py-2 border-b border-rose/10" href="/" onClick={() => setMobileOpen(false)}>الرئيسية</Link>
                                        <Link className="hover:text-rose transition-colors py-2 border-b border-rose/10" href="/products" onClick={() => setMobileOpen(false)}>المنتجات</Link>
                                        <Link className="hover:text-rose transition-colors py-2 border-b border-rose/10" href="/cart" onClick={() => setMobileOpen(false)}>السلة</Link>
                                        <Link className="hover:text-rose transition-colors py-2 border-b border-rose/10" href="#footer" onClick={() => setMobileOpen(false)}>تواصل معنا</Link>
                                    </div>
                                </div>
                                <div className="mt-auto pt-10 border-t border-rose/10">
